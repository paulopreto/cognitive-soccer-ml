...
```
Or with the virtualenv activated: `python run_pipeline.py`. This runs, in order: (1) data processing, (2) nested cross-validation evaluation, (3) overfitting validation (permutation test).

To run only the training step: `uv run python -m src.train_final_models`. To run only the permutation test: `uv run python -m src.validate_overfitting`.

## Repository structure (PEP 8, snake_case)

| Path | Description |
|------|-------------|
| `run_pipeline.py` | **Single entry point**: runs data processing → nested CV → permutation test. |
