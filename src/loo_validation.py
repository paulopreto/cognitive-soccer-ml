...
def load_best_params(path_csv, algorithm_name):
    ...
    Supports both English and Portuguese column names.
    """
    with open(path_csv, "r") as f:
        reader = csv.DictReader(f)
        for row in reader:
            algo = row.get("Algorithm Name") or row.get("Nome do Algoritmo")
            if algo == algorithm_name:
                params_str = row.get("Best Params") or row.get("Melhores Parametros")
...
def run_all_LOO_matrices(datasets_info, algorithm_name, path_save_img_final):
    ...
    for idx, info in enumerate(datasets_info):
        ...
        best_params = load_best_params(
            path_csv_p / f"{identifier}_parametros.csv", model_name_to_use
        )
