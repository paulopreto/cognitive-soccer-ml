    ...
    with pd.ExcelWriter(str(output_path), engine="openpyxl") as writer:
        ...
        for metric in results["Naive"][0].keys():
            df_metric = pd.DataFrame(
                {
                    "Naive": [
                        results["Naive"][i][metric]
                        for i in range(len(results["Naive"]))
                    ],
                    "Random forest": [
                        results["Random forest"][i][metric]
                        for i in range(len(results["Random forest"]))
                    ],
                    "KNN": [
                        results["KNN"][i][metric] for i in range(len(results["KNN"]))
                    ],
                    "Logistic Regression": [
                        results["Logistic Regression"][i][metric]
                        for i in range(len(results["Logistic Regression"]))
                    ],
                    "SVM": [
                        results["SVM"][i][metric] for i in range(len(results["SVM"]))
                    ],
                    "Neural network": [
                        results["Neural network"][i][metric]
                        for i in range(len(results["Neural network"]))
                    ],
                    "Xgboost": [
                        results["Xgboost"][i][metric]
                        for i in range(len(results["Xgboost"]))
                    ],
                }
            )
...
def aggregate_and_save_results(
    ...
    mean_results = {
        algo: compute_metric_means(res) for algo, res in results_dict.items()
    }
    path_save = Path(path_save)
