...
# ---------------------------
df = pd.read_csv(r'D:\Processamento_mestrado_Sports_Science\final_analysis\data\Dataset_cog_clusters.csv')
...
# Figure Creation
# ---------------------------
fig, axes = plt.subplots(nrows=len(features), ncols=len(comparisons), figsize=(22, 14), sharey='row')
...
for row_idx, feature in enumerate(features):
    for col_idx, (target_var, target_label) in enumerate(comparisons.items()):
        ...
        print(f"[{feature} vs {target_label}] -> Test: {test_name}, stat = {stat:.3f}, p = {p:.4f}")
        ...
        sns.boxplot(x='target_var_temp', y=feature, data=plot_data, ax=ax,
                    palette=palette, width=0.6, showfliers=False)
        sns.stripplot(x='target_var_temp', y=feature, data=plot_data, ax=ax,
        ...
        if feature == 'Acuracia Go':
            ...
            for x_pos in [0, 1]:
                ...
                for _ in outliers:
                    ...
                    ax.plot(x_pos + jitter, outlier_y, 'o',
                            markerfacecolor='red', markeredgecolor='black', markersize=5)

        # Annotate p-value in each subplot
        ax.text(0.5, ax.get_ylim()[0] + 0.02, f"p = {p:.3f}", ha='center', va='bottom', fontsize=9)
