...
def plot_vertical_clustered_with_violin(
    ax, df, data_col, cluster_col, y_label, title, invert_colors_labels=False
):
    """
    ...
    markers = ["o", "s"]
    marker_map = {
        cluster: markers[i % len(markers)] for i, cluster in enumerate(unique_clusters)
    }
    ...
    x_positions = np.random.uniform(
        -horizontal_jitter, horizontal_jitter, size=len(data)
    )
