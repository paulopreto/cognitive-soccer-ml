...
def save_table_as_image(data, columns, title, save_path, footer_text=None):
    ...
    table = ax.table(
        cellText=data.values, colLabels=columns, cellLoc="center", loc="center"
    )
...
def adicionar_maior_valor(dicionario_dataframes):
    ...
    for sheet_name, df in dicionario_dataframes.items():
        ...
        new_row[col_label] = (
            f"Max value: {max_value} (Row: {row_label}, Column: {col_label})"
        )
...
def extrair_salvar_medias(path_dir_combinacoes):
    ...
    (
        ...
        resultados_gc_dp,
        resultados_sg_dp,
    ) = criar_variaveis_vazias(metrics, columns)
            ...
            for _, _, files in os.walk(dir_path):
                ...
                for tentativa_file in result_files:
                    ...
                    for metric_name, df_metric in sheets.items():
                        ...
                        if prefix == "gf":
                            resultados_gf[metric_name] = pd.concat(
                                [resultados_gf[metric_name], mean_row], axis=0
                            )
                            resultados_gf_dp[metric_name] = pd.concat(
                                [resultados_gf_dp[metric_name], std_row], axis=0
                            )
                        elif prefix == "gs":
                            resultados_gs[metric_name] = pd.concat(
                                [resultados_gs[metric_name], mean_row], axis=0
                            )
                            resultados_gs_dp[metric_name] = pd.concat(
                                [resultados_gs_dp[metric_name], std_row], axis=0
                            )
                        elif prefix == "gc":
                            resultados_gc[metric_name] = pd.concat(
                                [resultados_gc[metric_name], mean_row], axis=0
                            )
                            resultados_gc_dp[metric_name] = pd.concat(
                                [resultados_gc_dp[metric_name], std_row], axis=0
                            )
                        elif prefix == "sg":
                            resultados_sg[metric_name] = pd.concat(
                                [resultados_sg[metric_name], mean_row], axis=0
                            )
                            resultados_sg_dp[metric_name] = pd.concat(
                                [resultados_sg_dp[metric_name], std_row], axis=0
                            )
